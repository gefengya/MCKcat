...
def extract_metrics_for_seed(logfile, outdir, seed_name):
    ...
    overall_mae = None
    overall_r2 = None
    output_file = os.path.join(outdir, 'ensemble_analysis_summary.csv')
    ...
    csv_file_path = os.path.join(outdir, 'ensemble_analysis_summary.csv')
    ...
    with open(csv_file_path, mode='a', newline='') as csvfile:
        ...
        if not file_exists:
            # 写入列标题
            writer.writerow(
                ['seed_name', 'model_r2_mean', 'model_r2_sem', 'overall_r2', 'model_mae_mean', 'model_mae_sem',
                 'overall_mae'])
        writer.writerow(
            [seed_name, model_r2_mean, model_r2_sem, overall_r2, model_mae_mean, model_mae_sem, overall_mae])
...
def calculate_and_save_summary(kcat_df, km_df, ki_df, output_file):
    ...
    def calc_metrics(df):
        print(df)
        individual_r2 = df['model_r2_mean']
        individual_mae = df['model_r2_sem']
        ensemble_r2 = df['overall_r2']
        ensemble_mae = df['model_r2_sem']
