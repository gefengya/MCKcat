...
def calculate_esm_for_sequence(sequence, device='cpu'):
    """
    计算单个序列的 ESM 表征
    ...
    representation = token_representations[0][1: len(sequence)+1]
...
# 假设这些是已定义的类型和函数
# from your_module import MoleculeDataset, MoleculeDatapoint, TrainArgs, PredictArgs, Logger
# from your_module import preprocess_smiles_columns, load_features, get_constraints, get_data_weights
# from your_module import get_task_names, get_inequality_targets, make_mol, filter_invalid_smiles
# from your_module import get_protein_embedder, calculate_esm_for_sequence, load_valid_atom_or_bond_features
...
def get_data(path: str,
             ...
             # 新增参数：控制特征缓存
             features_cache_path: str = None,
             regenerate_features: bool = False) -> MoleculeDataset:
    """
    获取CSV文件中的SMILES和目标值，支持特征生成、保存和加载功能。

    新增参数:
    :param features_cache_path: 特征缓存文件路径（.pkl.gz格式）
    :param regenerate_features: 是否强制重新生成特征，即使缓存文件存在
    """
    ...
    if args is not None:
        ...
        # 从args获取缓存相关参数
        if features_cache_path is None and hasattr(args, 'features_cache_path'):
            features_cache_path = args.features_cache_path
        if not regenerate_features and hasattr(args, 'regenerate_features'):
    ...
    if features_cache_path and not regenerate_features:
        try:
            if os.path.exists(features_cache_path):
                debug(f"从缓存文件加载特征: {features_cache_path}")
                with gzip.open(features_cache_path, 'rb') as f:
                    cached_features = pickle.load(f)
                    # 验证缓存内容
                    required_keys = ['features_data', 'phase_features', 'atom_features',
                                     'atom_descriptors', 'bond_features', 'bond_descriptors']
                    if not all(key in cached_features for key in required_keys):
                        raise ValueError("缓存文件格式不正确，缺少必要的键")
                debug(f"成功加载缓存特征")
        except Exception as e:
            debug(f"加载特征缓存失败: {str(e)}，将重新生成特征")
            regenerate_features = True

    # 加载或生成特征
    if cached_features and not regenerate_features:
        features_data = cached_features['features_data']
        phase_features = cached_features['phase_features']
        atom_features = cached_features['atom_features']
        atom_descriptors = cached_features['atom_descriptors']
        bond_features = cached_features['bond_features']
        bond_descriptors = cached_features['bond_descriptors']
    else:
        debug("开始生成特征...")
        # 加载特征
        if features_path is not None:
            ...
            for feat_path in features_path:
                features_data.append(load_features(feat_path))  # 每个都是 num_data x num_features
        ...
        if args is not None and args.atom_descriptors is not None:
            try:
                ...
                raise ValueError(f'加载或验证自定义原子描述符或特征失败: {e}')
        ...
        if args is not None and args.bond_descriptors is not None:
            try:
                ...
                raise ValueError(f'加载或验证自定义键描述符或特征失败: {e}')
        ...
        if features_cache_path:
            try:
                # 创建目录（如果需要）
                cache_dir = os.path.dirname(features_cache_path)
                if cache_dir and not os.path.exists(cache_dir):
                    os.makedirs(cache_dir, exist_ok=True)

                # 保存特征数据
                debug(f"将特征保存到缓存文件: {features_cache_path}")
                with gzip.open(features_cache_path, 'wb') as f:
                    pickle.dump({
                        ...
                        'timestamp': np.datetime64('now'),  # 添加时间戳
                        'smiles_columns': smiles_columns,
                        'max_data_size': max_data_size
                    }, f, protocol=pickle.HIGHEST_PROTOCOL)
                debug(f"特征已成功保存到缓存")
            except Exception as e:
                debug(f"保存特征缓存失败: {str(e)}")

    # 加载约束
    if constraints_path is not None:
        constraints_data, raw_constraints_data = get_constraints(
            path=constraints_path,
            target_columns=args.target_columns if args else None,
            save_raw_data=args.save_smiles_splits if args else False
        )
    else:
    ...
    with open(path) as f:
        ...
        if any([c not in fieldnames for c in smiles_columns]):
            raise ValueError(
                f'数据文件不包含所有提供的SMILES列: {smiles_columns}。数据文件字段名是: {fieldnames}')
        if any([c not in fieldnames for c in target_columns]):
            raise ValueError(
                f'数据文件不包含所有提供的目标列: {target_columns}。数据文件字段名是: {fieldnames}')

        all_smiles, all_sequences, all_targets, all_atom_targets, all_bond_targets = [], [], [], [], []
        ...
        for i, row in enumerate(tqdm(reader)):
            ...
            if args and args.smoke_test:
                if smoke_test_counter > 100:
                    break

            smiles = [row[c] for c in smiles_columns]
            pdbpath = row.get('pdbpath')
            pdbname = pdbpath.split("/")[-1] if pdbpath else None

            # 处理蛋白质记录和特征
            protein_record = None
            if protein_records is not None and pdbname:
                try:
                    protein_record = protein_records[pdbname]

                    if protein_record is not None and sequence_feat_getter:
                        sequence_features, _ = sequence_feat_getter(
                            protein_record['seq'],
                            name=pdbname,
                            device='cpu'
                        )
                        protein_record['esm2_feats'] = sequence_features[0]  # 去除批次维度
                except KeyError:
                    debug(f"在protein_records中未找到键 {pdbname}。正在计算ESM表示...")
                    # 从行中获取蛋白质序列
                    sequence = row.get('sequence')
                    if sequence is not None:
                        # 清理序列
                        sequence = sequence.replace('X', '').replace('U', '')
                        esm_representation = calculate_esm_for_sequence(sequence, device='cpu')
                        protein_record = {'seq': sequence, 'coords': esm_representation}
                        protein_record['esm2_feats'] = esm_representation
                    else:
                        debug(f"在 {pdbname} 的行中未找到序列。跳过此行数据。")
                        continue
            ...
            for column in target_columns:
                ...
                if value in ['', 'nan']:
                    ...
                    if loss_function == 'bounded_mse':
                        ...
                        raise ValueError(
                            '在目标数据中发现不等式。要使用不等式目标(>或<)，必须使用回归损失函数bounded_mse。')
                elif '[' in value or ']' in value:
                    value = value.replace('None', 'null')
                    target = np.array(json.loads(value))
                    if len(target.shape) == 1 and args and column in args.atom_targets:  # 原子目标保存为1D列表
                        atom_targets.append(target)
                        targets.append(target)
                    elif len(target.shape) == 1 and args and column in args.bond_targets:  # 键目标保存为1D列表
                        bond_targets.append(target)
                        targets.append(target)
                    elif len(target.shape) == 2:  # 键目标保存为2D列表
                        ...
                        raise ValueError(f'无法识别 {path} 中 {column} 列的目标。')
                else:
                    # 清理并转换为浮点数
                    clean_value = value.strip('"')
                    targets.append(float(clean_value))

            # 检查是否所有目标都是None，如果是则跳过
            ...
            # 添加特征数据
            if features_data is not None and i < len(features_data):
                all_features.append(features_data[i])
            else:
                all_features.append(None)

            if phase_features is not None and i < len(phase_features):
                all_phase_features.append(phase_features[i])
            else:
                all_phase_features.append(None)

            if constraints_data is not None and i < len(constraints_data):
                all_constraints_data.append(constraints_data[i])
            else:
                all_constraints_data.append(None)

            if raw_constraints_data is not None and i < len(raw_constraints_data):
                all_raw_constraints_data.append(raw_constraints_data[i])
            else:
                all_raw_constraints_data.append(None)

            if data_weights is not None and i < len(data_weights):
            ...
            if lt_targets is not None and i < len(lt_targets):
        ...
        data = MoleculeDataset([
            MoleculeDatapoint(
                ...
                atom_features=atom_features[i] if atom_features is not None and i < len(atom_features) else None,
                atom_descriptors=atom_descriptors[i] if atom_descriptors is not None and i < len(
                    atom_descriptors) else None,
                bond_features=bond_features[i] if bond_features is not None and i < len(bond_features) else None,
                bond_descriptors=bond_descriptors[i] if bond_descriptors is not None and i < len(
                    bond_descriptors) else None,
    ...
    if skip_invalid_smiles:
        ...
        if len(data) < original_data_len:
            debug(f'警告: {original_data_len - len(data)} 个SMILES无效。')
