...
class MoleculeModel(nn.Module):
    ...
    def create_protein_model(self, args: TrainArgs) -> None:
        ...
        self.rotary_embedder = RotaryEmbedding(dim=args.seq_embed_dim // 3)
    ...
    def forward(
        ...
        def match_tensor_sizes(tensor1, tensor2, dim):
            size1 = tensor1.size(dim)
            size2 = tensor2.size(dim)
            # print(tensor1.shape, tensor2.shape)
            # print(size1, size2, tensor2.size)
            if size1 > size2:
                # 裁剪 tensor1
                indices = [slice(None)] * tensor1.dim()
                indices[dim] = slice(0, size2)
                tensor1 = tensor1[tuple(indices)]
            elif size2 > size1:
                # 填充 tensor1
                padding = [0] * (tensor1.dim() * 2)
                padding[-2 * (dim + 1)] = size2 - size1
        ...
        def seq_to_tensor(seq):
            letter_to_num = {'C': 4, 'D': 3, 'S': 15, 'Q': 5, 'K': 11, 'I': 9,
                             ...
                             'N': 2, 'Y': 18, 'M': 12}
            valid_chars = [a for a in seq if a in letter_to_num]
            seq = torch.as_tensor([letter_to_num[a] for a in valid_chars],
        ...
        if self.is_atom_bond_targets:
            ...
            if not self.args.skip_protein and not self.args.protein_records_path is None:
                ...
                if self.args.add_esm_feats:
                    esm_feature_arr, seq_outs = match_tensor_sizes(esm_feature_arr, seq_outs, dim=1)
